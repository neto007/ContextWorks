...
def run_cwe_checker(
    ...
    job_id = os.environ.get("WM_JOB_ID", "local_test_cwe_checker")
    ...
    cmd = [
        ...
        "-v", f"{abs_path}:/app_data/{filename}",
        image, "/app_data/{filename}"
    ]
    
    print(f"DEBUG: Running cwe_checker on {filename}...", file=sys.stderr, flush=True)
