...
def run_binwalk(
    ...
    Args:
        ...
        extract: Whether to attempt extraction (-e).
    """
    image = "python:3-alpine"
    job_id = os.environ.get("WM_JOB_ID", "local_test_binwalk")
    ...
    # binwalk needs some system deps for full extraction features
    setup_and_run = (
        "apk add --no-cache build-base libtool autoconf automake zlib-dev xz-dev >/dev/null && "
        "pip install binwalk >/dev/null 2>&1 && "
        f"binwalk {args} /app_data/{filename}"
    )
    ...
    print(f"DEBUG: Running binwalk on {filename}...", file=sys.stderr, flush=True)
...
def main(file_content: bytes = b"", filename: str = "binary_file"):
    ...
    print(json.dumps(run_file_path(file_content, filename), indent=2), flush=True)
