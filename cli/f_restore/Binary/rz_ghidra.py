...
def run_rz_ghidra(
    ...
    """
    Run rz-ghidra (via rizin).
    
    Args:
        file_content: Binary file content (bytes or base64).
        filename: Binary filename.
    """
    image = "rizinorg/rizin:latest"
    job_id = os.environ.get("WM_JOB_ID", "local_test_rz_ghidra")
    ...
    # Run rizin with ghidra decompiler on the binary
    # 'pdg' command in rizin uses ghidra
    setup_and_run = (
        f"rizin -q -c 'pdg' /app_data/{filename}"
    )
    ...
    print(f"DEBUG: Running rz-ghidra on {filename}...", file=sys.stderr, flush=True)
