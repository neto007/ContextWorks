...
def run_app_info_scanner(
    ...
    job_id = os.environ.get("WM_JOB_ID", "local_test_app_info_scanner")
    ...
    setup_and_run = (
        ...
        "git clone https://github.com/kelvinBen/AppInfoScanner.git /app >/dev/null && "
        "cd /app && "
        "pip install -r requirements.txt >/dev/null 2>&1 && "
        f"python app.py -i /app_data/{filename}"
    )
    ...
    print(f"DEBUG: Running AppInfoScanner on {filename}...", file=sys.stderr, flush=True)
...
def main(file_content: bytes = b"", filename: str = "test.apk"):
    ...
    print(json.dumps(run_app_path(file_content, filename), indent=2), flush=True)
