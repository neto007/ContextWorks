...
def run_droidefense(
    ...
    job_id = os.environ.get("WM_JOB_ID", "local_test_droidefense")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        setup_and_run = (
            ...
            "git clone https://github.com/droidefense/engine.git /app >/dev/null && "
            "cd /app && "
            "pip install -r requirements.txt >/dev/null 2>&1 && "
            f"python engine.py -i /app_data/{filename}"
        )
        ...
        print(f"DEBUG: Running Droidefense on {filename}...", file=sys.stderr, flush=True)
...
def main(file_content: bytes = b"", filename: str = "test.apk"):
    ...
    print(json.dumps(run_droidefense(file_content, filename), indent=2), flush=True)
